...
export type PageSliderPageMaker = React.Component| React.FC;
...
const PageSlider:React.FC<PageSliderProps> = (props)=>{
  ...
  const {scrollXRef=defaultScrollXRef, itemWidth=width, data, ...rest} = props;
  ...
  const next = ()=>{
    // call scroll to next item
    console.log('NEXT::',{'indexRef.current':indexRef.current,'data.length':data.length, 'next':indexRef.current+1 })
  ...
  const prev = ()=>{
    console.log('PREV::',{'indexRef.current':indexRef.current,'data.length':data.length, 'next':indexRef.current-1 })
